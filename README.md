...
Спроэктируем API магазин по RESTFUL методологии
...
Описанные выше сущности реализуем в реляционной БД PostgreSQL.

Реализуем следующие популярные виды HTTP запросов (GET, POST, PUT, DELETE, PATCH).
...
Покроем все методы спецификацией OpenAPI. Swagger спецификация должна отдаваться по пути `http://localhost:{YourPort}/swagger/index.html`

Pеализуем паттерна **Repository** в качестве прослойки доступа к данным.
